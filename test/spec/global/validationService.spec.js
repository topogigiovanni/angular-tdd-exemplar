describe('The validationService logic', function() {
	...
	describe('the contact validationService function', function() {
		...
		it('should return false on a contact with no name', function() {
			...
			var contactWithNoName = {
				age: 36,
				occupation: 'Janitor',
				email: 'janitor@planetexpress.com'
			};

			expect(validationService.isValidContact(contactWithNoName)).to.be.false;
		...
		it('should return false on a contact with no email', function() {
			...
			assert.isFalse(validationService.isValidContact(contactWithNoEmail));

		});

		it('should return true on a contact with no age, it is optional', function() {

			var contactWithNoAge = {
				name: 'Scruffy',
				occupation: 'Janitor',
				email: 'janitor@planetexpress.com'
			};

			expect(validationService.isValidContact(contactWithNoAge)).to.be.true;

		});

		it('should return true on a contact with no occupation, because that is optional', function() {

			var contactWithNoOccupation = {
				name: 'Scruffy',
				age: 25,
				email: 'janitor@planetexpress.com'
			};

			expect(validationService.isValidContact(contactWithNoOccupation)).to.be.true;
