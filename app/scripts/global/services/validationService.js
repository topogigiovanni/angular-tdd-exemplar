angular.module('AddressBook.Global')
	.service('validationService', function() {
		...
		function isValidContact(contact) {
			...
			if (!contact.name || contact.name && !isValidName(contact.name)) {
				valid = false;
			}

			if (!contact.email || contact.email && !isValidEmail(contact.email)) {
				valid = false;
			}
			...
			if (contact.occupation && !isValidOccupation(contact.occupation)) {
				valid = false;
			}
